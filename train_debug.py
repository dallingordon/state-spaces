...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        #################### i think we make the task here something special, hydra, gautam.  
        self.task = utils.instantiate(
            tasks.registry, self.hparams.task, dataset=self.dataset, model=self.model
        )
        print(self.task) #########if we make it so that the decoder is convolved, and outputs one per transformer "step"
        # Create encoders and decoders
        encoder = encoders.instantiate(
            encoder_cfg, dataset=self.dataset, model=self.model
        )
        decoder = decoders.instantiate(  ########src/task/decoders!! line 37 has a mode last.  This is the output from s4 to use as transformer input.
        ...
        #print('the self.task.encoder',self.task.encoder)
        #print('the encoder config',encoder_cfg)
        #print("The docoder cfg",decoder_cfg)
        #print(decoder, " just decoder, no self")
        ###SequenceDecoder(
        ###(0): SequenceDecoder(
        ###    (output_transform): Linear(in_features=256, out_features=10, bias=True)
        ## )
        ##)
        #print(self.task.decoder, 'self.task.decoder') ##this is none?
    ...
    def forward(self, batch):
        ...
        # 256 is d_model.  can i change that on command?
        x, y, *z = batch # z holds extra dataloader info such as resolution
        print("Input shape and targets: ",x.shape,y.shape) #torch.Size([50, 16000, 1]) torch.Size([50]) line 295
        if len(z) == 0:
            z = {}
        else:
            assert len(z) == 1 and isinstance(z[0], dict), "Dataloader must return dictionary of extra arguments"
            z = z[0]
        ###This is where we unfold, so, 50,16,1000,1
        ##call s4 with time step of 1000, how do we do this most efficiently.
        #encoder and model get convolved
        x, w = self.encoder(x, **z) # w can model-specific constructions such as key_padding_mask for transformers or state for RNNs
        print(x.shape, 'line 303; encoder output') # torch.Size([50, 16000, d_model (256)]) line 303; encoder output w is a dict....
        x, state = self.model(x, **w, state=self._state)
        print(x.shape,state,'line 305; model output and state shape') #state is a list? torch.Size([50, 16000, d_model(256)]) [None] line 305; model output and state shape
        self._state = state
        ##after convolution we should have 50 (batch) x 16 x 1000 x d_model (latent)
        ##flatten last 2 dims for trainsformer input
        ##turns into a linear layer (this is our pseudo decoder 50 (batch) x 16 x transformer_input
        ## flatten trans out for the final change
        ##transformer turns it into 50 x 10
        print(self.decoder, z)  #U.PassthroughSequential(decoder, self.task.decoder) it isnt linear, it is this sequential thing...
        ## /src/models/nn/utils.py in the code.  this might be okay.    is this the discretization?
        print(x.shape,'line 327 decoder input!!')
        x, w = self.decoder(x, state=state, **z) #this is classifying AND decoding...make it spit out the latent input to the transformer.
        print(x.shape, 'line 308 decoder output') ##torch.Size([50, 10]) line 308 decoder output
        """
        it does one call and goes from 
        """
...
def train(config):
    print('The config in all its glory:', '\n\n\n\n', config)
