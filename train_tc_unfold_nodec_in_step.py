...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        self.segment_length = 1000 #these should be setup via HYDRA gautam
        self.stride = 1000
        self.total_segments = 16 ##this will have to be derived :(
        self.transformer_dim = 100
        self.num_classes = 10 #the 10 should be from sc
        self.transformerInputLinear = nn.Sequential(nn.Linear(self.segment_length * 256, self.transformer_dim)
                                                    , nn.ReLU())        #the 256 is d_model.  wire this up gautam
        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=self.transformer_dim
                                       , nhead=10
                                       , dim_feedforward=1000 #ask sadie
                                       , dropout=0.1 #default
                                       , batch_first=True)
            , num_layers=5) #hyperparam gautam
    ...
    def forward(self, batch):
        ...
        x = x.view(-1,x.shape[-2],x.shape[-1]) #stack in batch dim
        x, state = self.model(x, **w, state=self._state) #s4 call
        x = x.view(x.shape[0],-1)  #flattens for the linear input should be 800 x 256_000
        x  = self.transformerInputLinear(x) #prepare for transformer (discretization?)
        x = x.view(-1,self.total_segments,self.transformer_dim) #get original batch back
        x = self.transformer_encoder(x) #pass through transformer, input and output the same
        x = x.view(x.shape[0],-1) #flatten for classifications
        x = self.transformerClassifier(x) #classifier shape, activation? softmax maybe?

        #raise Exception("Stop.  hammertime. not debug") ##this is just to stop, for debugging only
        #self._state = state
    ...
    def step(self, x_t):
        ...
        # x_t = x_t[:, 0, ...]
        ##x_t, *_ = self.decoder.step(x_t, state=state) #removed by dg 9/22
        return x_t
