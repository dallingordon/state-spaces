...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        ###Additions DG 9-18vvvvvvvvvv
        print("the params config i hope: ",self.hparams) # i want a copy of this plz

        #print("d_model:",self.hparams.model.d_model)
        #print("batch_size:",self.hparams.loader.batch_size)
        self.batch = self.hparams.loader.batch_size
        self.seq_length = self.hparams.dataset.length
        self.s4_dmodel = self.hparams.model.d_model
        ##get this from decoder....
        self.num_classes = 10 #couldn't find this... where do i find this mama

        # our model specific hparams:
        # for the convolution
        self.segment_length = self.hparams.tc.conv.segment_length
        self.stride = self.hparams.tc.conv.stride

        # for the transformer
        self.transformer_dim = self.hparams.tc.transformer.trans_dim
        self.transformer_ff_dim = self.hparams.tc.transformer.ff_dim
        self.tranformer_num_layers = self.hparams.tc.transformer.num_layers
        self.transformer_num_heads = self.hparams.tc.transformer.num_heads
        self.transformer_dropout = self.hparams.tc.transformer.dropout

        test_zeros = torch.zeros(self.batch, self.seq_length, self.s4_dmodel)
        test_unf_tensor = test_zeros.unfold(1, self.segment_length, self.stride)
        print(test_unf_tensor.shape)
        self.total_segments = test_unf_tensor.shape[1]

        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=self.transformer_dim
                                       , nhead=self.transformer_num_heads
                                       , dim_feedforward=self.transformer_ff_dim  # ask sadie
                                       , dropout=self.transformer_dropout
                                       , batch_first=True)
            , num_layers=self.tranformer_num_layers)

        self.transformerClassifier = nn.Sequential(nn.Linear(self.total_segments * self.transformer_dim , self.num_classes)
                                                   ,nn.Softmax(dim=-1)) #dim is last, just 2 dimensions, 0 is batch
        ###Additions DG 9-18^^^^^^^^^^
        ...
        #decoder_cfg[1]['d_output'] = 115  #TypeError: SequenceDecoder.__init__() got multiple values for argument 'd_output'.  maybe i mute it in theirs?
        #print("decoder cfg",decoder_cfg) #decoder cfg [None, {'_name_': 'sequence', 'mode': 'pool'}]
        #this needs to edit the seq lenght (should be self.segment_length) and the output dim (should be self.transformer_dim = 40)
        #raise Exception("Stop.  look at the decoder cfg")
        # Instantiate model
        #print("model stuff:",self.hparams.model)
        ...
        ######https://github.com/dallingordon/state-spaces/blob/main/src/tasks/decoders.py#L37C23-L37C23
        ##run the config for the normal model and see how s4 is setup, use that, fix the dimensions.
        ## tack on the sequence decoder, but override the dimensions.  this is the
        #print("self.model gets passed to decoder, and d_ouptut used.  can i mess with that?", self.model)
        #fix this so the d_model is the transdim here.
        ...
        #print(dir(decoder))
        #print("decoder 0",decoder[0]) #.output_transform.out_features = self.transformer_dim #dg 9/27 This fixes the decoder to feed to the transformer, not direct to output.  worked?! WORKED!!

        #mess with the decoder here?

        #print("print just the decoder:",decoder)
        """
        print just the decoder: SequenceDecoder(
            (0): SequenceDecoder(
            (output_transform): Linear(in_features=256, out_features=10, bias=True)
            )
            )
        """
        ...
        ###this i think makes the decoder work with any transform dim! it did :) dg
        self.decoder[0].output_transform = nn.Linear(self.s4_dmodel,self.transformer_dim)

        print("output transform",self.decoder[0].output_transform) #this seems to be insufficient.

        ##this is an init test for the added components.  dg 10/10
        #for layer in self.transformer_encoder.modules():
        #    if isinstance(layer, nn.Linear):
        #        # Xavier/Glorot initialization for linear layers
        #        nn.init.xavier_uniform_(layer.weight)

        #for layer in self.transformerClassifier.modules():
        #    if isinstance(layer, nn.Linear):
        #        # Xavier/Glorot initialization for linear layers
        #        nn.init.xavier_uniform_(layer.weight)

        for layer in self.decoder[0].modules(): #maybe this inits it?
            if isinstance(layer, nn.Linear):
                # Xavier/Glorot initialization for linear layers
                nn.init.xavier_uniform_(layer.weight)
    ...
    def forward(self, batch):
        ...
        #print("line 302, Input data shape: ", x.shape) #Input data shape:  torch.Size([50, 16000, 256])
        x = x.unfold(1, self.segment_length, self.stride) #this in conjuction with the stack is how I am doing the convolution
        #print("unfold operation output: ", x.shape) #unfold operation output:  torch.Size([50, 16, 256, 1000])
        total_segments = x.shape[1]
        #print("total segments: ",self.total_segments) #16
        x = x.permute(0, 1, 3, 2) ##swap axes so the d_model is last
        #print("permute: ",x.shape) # permute:  torch.Size([50, 16, 1000, 256])
        x = x.reshape(-1,x.shape[-2],x.shape[-1])
        #DALLIN SEARCH THIS ERROR#######RuntimeError: view size is not compatible with input tensor's size and stride (at least one dimension spans across two contiguous subspaces). Use .reshape(...) instead.
        #print("batch stack: ",x.shape) # batch stack:  torch.Size([800, 1000, 256])  ##ValueError: Expected input batch_size (800) to match target batch_size (50)

        x, state = self.model(x, **w, state=self._state)
        #print("s4 out: ", x.shape) #s4 out:  torch.Size([800, 1000, 256]) WORKED!!!!!!!!!! we had 50 input samples, but each one had 16 segments, that is the 800
        ##print(state) #[None, None, None, None, None, None, None, None]
        ##now,reshape for transormer input.  50 batches, confirm in nb
        x, w = self.decoder(x, state=state, **z) #takes in seqxd_model, should output b (800) x 100
        ##i deleted from here the flatten and transformerlinear.
        #print("decoder output",x.shape) #decoder output torch.Size([8000, 10]) # alright, i need to override the class somehow..
        x = x.reshape(-1,self.total_segments,self.transformer_dim) # should be 50 x 16 x 100 (trans dim)
        #print("rebatched post conv",x.shape) #rebatched post conv torch.Size([50, 16, 100])
        x = self.transformer_encoder(x) #this should be 50 x 16 x 100
        #print("transformer output: ", x.shape) #transformer output:  torch.Size([50, 16, 100])
        x = x.reshape(x.shape[0],-1) #should be 50 x 1600
        #print("transformer output flatten: ", x.shape) #transformer output flatten:  torch.Size([50, 1600])
        x = self.transformerClassifier(x) #should be 50 x 10...
        #print("did we do it?:", x.shape) #did we do it?: torch.Size([50, 10])
        #print(self.hparams) # worked in setup, skipping
        #raise Exception("Stop.  hail hydra") ##this is just to stop, for debugging only
        #self._state = state

        return x, y, w

    def step(self, x_t):
        x_t, *_ = self.encoder(x_t) ##does this need to be convolved as well? that is the next step
        x_t, state = self.model.step(x_t, state=self._state) #what is model.step?
        self._state = state
        # x_t = x_t[:, None, ...] # Dummy length
        # x_t, *_ = self.decoder(x_t, state=state)
        # x_t = x_t[:, 0, ...]
        x_t, *_ = self.decoder.step(x_t, state=state)  ##Turned this off.  I think i need to track the state.
        return x_t
