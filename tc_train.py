...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
       ...
       ################################vvvvmy editsvvvv############################################
        ...
        self.stride = 250
        self.sequence_length = 16_000  # get seq length of s4
        self.num_segments = (self.sequence_length - self.segment_size) // self.stride + 1

        self.classifier = nn.Linear(self.num_segments*self.d_model_c,self.out_classes)
        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=self.d_model_c
                                       , nhead=self.nhead
    ...
    ################################^^^^my edits^^^^^##############################################
        ...
        self.decoder = U.PassthroughSequential(decoder, self.task.decoder) #is this the classifier?S
    ...
    def forward(self, batch):
        ...
        print(x.shape)
        provided_batch_size, provided_sequence_length = x.shape
        assert(self.sequence_length == provided_sequence_length)
        c_output = torch.zeros(self.batch_size, self.num_segments, self.d_model_c, device=x.device)

        for i in range(num_segments):
            start_idx = i * self.stride
            end_idx = start_idx + self.segment_size
            input_segment = x[:, start_idx:end_idx] # B x segment_size
            # ##segment_output = self.convolved_model(input_segment).unsqueeze(1) ##adds a ghost dimension after batch
        xx, w = self.encoder(input_segment, **z) # w can model-specific constructions such as key_padding_mask for transformers or state for RNNs
        xx, state = self.model(xx, **w, state=self._state)
        self._state = state
        segment_output, w = self.decoder(xx, state=state, **z)   #i think decoder may need to be outside? is this the classifier? could just turn it off.
            ##actually, i probably need to override this.  we'll find out.
        c_output[:, i, :] = segment_output[:, -1, :] #conv operation output

        t_output = self.transformer_encoder(c_output) #transformer output
        ##flatten and classify
        t_output = t_output.view(batch_size, -1)
        output = torch.sigmoid(self.classifier(t_output))
