...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        self.total_segments = test_unf_tensor.shape[1]
         ...
         #the 10 should be from sc
        self.transformerInputLinear = nn.Sequential(nn.Linear(self.segment_length * self.s4_dmodel, self.transformer_dim)
                                                    , nn.ReLU())        #the 256 is d_model.  wire this up gautam
    ...
    def forward(self, batch):
        ...
        x = x.reshape(x.shape[0],-1)  #flattens for the linear input should be 800 x 256_000
        print("flatten for linear transformer input:" ,x.shape)
        x  = self.transformerInputLinear(x)
        print("transformerInputLinear output",x.shape) #transformerInputLinear output torch.Size([800, 100])
    ...
    def step(self, x_t):
        ...
        # x_t = x_t[:, 0, ...]
        ###################################### what is this doing? i don't do decoder in the forward, this maybe should be commented out?
